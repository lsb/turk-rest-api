...
def consume_assignments!(db, queue_endpoint, queue_access, queue_secret, turk_endpoint, turk_access, turk_secret)
  ...
  assignment_ids_hit_ids.each {|assignment_id, hit_id|
    ...
    rejection_notice = lambda { "Hello, thank you for your work, but of the questions in the HIT that we knew the answers to, you got #{a.fetch("gold_standards_percent_correct")}% correct when we were looking for #{knownAnswerQuestions.fetch("percentCorrect")}% correct." }
